...
const { response } = require('express');
  ...
  router.get('/refresh', function(req, res, next) {
    ...
    console.log("client", req.spotify._credentials.clientId)
    console.log("secret", req.spotify._credentials.clientSecret)
    // console.log("url: ", `grant_type=authorization_code&code=${encodeURIComponent(code)}&redirect_uri=${encodeURIComponent("https://randofy.vercel.app/")}&client_id=${req.spotify._credentials.clientId}&client_secret=${req.spotify._credentials.clientSecret}`);
    axios.post("https://accounts.spotify.com/api/token",
    `grant_type=refresh_token&refresh_token=${encodeURIComponent(refresh)}`,
    {
        'Authorization': 'Basic ' + Buffer.from(req.spotify._credentials.clientId + ":" + req.spotify._credentials.clientSecret).toString('base64'),
        'Content_Type': 'application/x-www-form-urlencoded',
        'Access-Control-Allow-Header': '*',
        'Cache-Control': 'no-cache',
    })
